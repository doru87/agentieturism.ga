...
<html>
  ...
  <body>
        ...
        <form action="adauga_circuit.php" method="post" enctype="multipart/form-data">
          <table align="center" width="700px" bgcolor="orange" border="2">
              ...
              <tr>
                  <td align="right"><b>Destinatia circuitului (continent):</b></td>
                  <td>
               ...
               <tr>
                  <td align="right"><b>Destinatia circuitului (tara):</b></td>
                  <td>
              ...
              <tr>
                  <td align="right"><b>Destinatia circuitului (oras):</b></td>
                  <td>
              ...
              <tr>
                  ...
                  <td><input type="text" name="pret_circuit" size="10"/></td>
              </tr>
              ...
              <tr>
               ...
               <tr>
                  <td colspan="7"><input type="submit" name="adauga_circuit" value="Adauga Circuit" /></td>
               </tr>
  ...
  $(document).ready(function(){
	    ...
	    $('#tara').on("change",function () {
	        var tara = $(this).find('option:selected').text();
	        $.ajax({
	            url: "ajax.php",
	            type: "POST",
	            data: {tara:tara},
...
<?php
  if(isset($_POST['adauga_circuit'])) {
    ...
    if (!file_exists($_SERVER["DOCUMENT_ROOT"]."/".circuite."/".$tara)) {
        mkdir($_SERVER["DOCUMENT_ROOT"]."/".circuite."/".$tara, 0777, true);
...
//     }
    ...
    {
        $uploadfile="$oras/".$_FILES["poza_articol"]["tmp_name"][$i];
        $document_root=$_SERVER['DOCUMENT_ROOT'];

            move_uploaded_file($_FILES["poza_articol"]["tmp_name"][$i], "$document_root"."/cicuite"."/"."$tara/".$_FILES["poza_articol"]["name"][$i]);

        $poza_articol = implode(",",$_FILES['poza_articol']['name']);
    }
   
            $insereaza_articol = "insert into destinatie_circuit (titlu, continent, tara, oras, perioada, continut, itinerar, tarife_cazare, pret, servicii_incluse, atractii_turistice, data_publicarii,poza) values
            ('$titlu_articol','$continent','$tara','$oras','$perioada_circuit','$continut_articol','$itinerar','$tarife_cazare','$pret','$servicii_incluse','$atractii_turistice','$data_publicarii','$poza_articol')";
